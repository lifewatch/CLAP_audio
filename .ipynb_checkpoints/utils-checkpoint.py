...
def eval_pytorch_model(model, dataloader, metric_factory, device, similarity_matrix,similarity_matrix_distance,similarity_matrix_speed,similarity_matrix_activity, similarity_matrix_type,desc,weights,comment="", ids=None):
    model.eval()
    total_loss = 0.
    steps = 0
    ...
    if ids is not None:
        ...
        csv_filename = f'predicted_true_values_{timestamp}_{weights_str}_{comment}.csv'
    ...
    return total_loss, np.mean(metrics),np.mean(metrics_distance),np.mean(metrics_speed),np.mean(metrics_activity), np.mean(metrics_type)
...
def similarity(label_to_id,device,distance_weight = 0, speed_weight = 0,activity_weight = 0,vessel_type_weight=0):

    ...
    def train_clap(self, model_path='davidrrobinson/BioLingual', epochs=1, lr=1e-5,
        ...
        for epoch in range(epochs):
            ...
            for x, y in tqdm(dataloader_train, desc='train'):
                ...
                # Print the accuracy
                print(f"Training metrics: {np.mean(metrics)}")
            ...
            val_loss, valid_metric=u.eval_pytorch_model_dclde(model, dataloader_val, device,"val",weights,ids,similarity_matrix, comment="")
            ...
            if min(valid_metric_previous, valid_metric) > valid_metric_best:
                print("breaking early")
                break
            
            valid_metric_previous=valid_metric
        ...
        test_loss, test_metric=u.eval_pytorch_model_dclde(model, dataloader_test, device,"test",weights,ids,similarity_matrix, comment="")
        ...
        log_file.write(log_message + '\n')
        log_file.flush()
        optimizer = optim.Adam(params=model.parameters(), lr=lr/10)
        scheduler = lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)
        for epoch in range(epochs):
            # print(f'epoch = {epoch}', file=sys.stderr)
            sys.stderr.write('epoch = {}\n'.format(epoch))
            ...
            for x, y in tqdm(dataloader_dclde_train, desc='train'):
                optimizer.zero_grad()
                # print("optimizer")
                x = x.to(device)
                y = y.to(device)
            ...
            log_message = json.dumps({
                'epoch_dclde': epoch,
                'train_dclde': {
                    'loss_dclde': (train_loss / train_steps).cpu().item(),
                    'metric_dclde': np.mean(metrics),
                },
                })
            log_file.write(log_message + '\n')
            log_file.flush()

        test_loss_dclde, test_metric_dclde=u.eval_pytorch_model_dclde(model, dataloader_dclde_test, device,"test_dclde",weights,ids,similarity_matrix, comment="")

        
        log_message = json.dumps({
            'test': {
                'loss': test_loss_dclde,
                'metric': test_metric_dclde,
            }
        })
        ...
        #     comment=comment,
        #     ids=ids)
        # u.eval_pytorch_model_dclde(model, dataloader_test, device,"test",weights,ids, comment="")
        # u.eval_pytorch_model_dclde(model, dataloader_dclde_test, device,"dclde",weights,ids, comment="")
